...
##   WCFP_plantlist_final_2026-06-02_redlist-patched.xlsx
##                                                     the taxon list + Red List codes
##                                                     (produced by
##                                                      patch_plantlist_redlist_2026-09-18.R
...
# load wcfp
#
# This is the Red List-patched working copy, NOT the original
# WCFP_plantlist_final_2026-06-02.xlsx. Two taxa - WCFP_ID 25484 Citrus
# warburgiana (DD) and 26317 Sinapis pubescens (LC) - are assessed in the
# published figshare release but blank in the original working copy, and a
# blank is read as NE (Not Evaluated) further down, so both were being
# reported as unassessed on every map and in every workbook.
#
# The patched file is produced by patch_plantlist_redlist_2026-09-18.R, which
# copies those two values FROM the figshare release rather than hard-coding
# them, and verifies that exactly two cells differ from the original. A
# provenance note sits beside it:
#   WCFP_plantlist_final_2026-06-02_redlist-patched_PROVENANCE.txt
#
# Everything else is identical to the original, so this is a drop-in swap -
# re-point this one line to run against the unpatched list.
wcfp_plantlist <- read_excel(
  file.path(inputs_dir, "WCFP_plantlist_final_2026-06-02_redlist-patched.xlsx"))
